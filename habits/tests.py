...
class HabitTestCase(APITestCase):
    ...
    def test_habit_delete(self):
        ...
        self.assertEqual(
            Habit.objects.all().count(), 0  # Так как из базы данных был удален объект 1 (создано 1)
        )
    ...
    def test_habit_retrieve(self):
        ...
        self.assertEqual(
            data.get('bonus'), self.habit.bonus    # Второе значение можно написать как "Получишь 100 рублей"
        )
        self.assertEqual(
            response.json(), {
                                     'id': self.habit.id,
    ...
    def test_habit_update(self):
        ...
        self.assertEqual(
            data.get('bonus'), "Test_bonus"  # Так как были внесены изменения "Тест урока" поменялся на "Test"
        )
